...
class ContactMatrix:
    ...
    def _write_contig_info_metacc(self):
        with open(os.path.join(self.metacc_folder , 'tmp' , 'contig_info_metacc.csv'),'w') as out:
    ...
    def set_primary_acceptance_mask(self, min_len=None, min_sig=None, max_fold=None, update=False):
        ...
        acceptance_mask = np.ones(self.total_seq_bin3c, dtype=np.bool)
    ...
    def prepare_seq_map(self, norm=True, bisto=True, mean_type='geometric'):
        ...
        _map = self.seq_map_bin3c.astype(np.float)
    ...
    def _get_sites(self):
        _sites = np.array([si.sites for si in self.seq_info_bin3c], dtype=np.float)
