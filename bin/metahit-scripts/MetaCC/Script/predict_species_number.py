...
def gen_bestk(dir_path , contig_file):
    ...
    fragURL = os.path.join(f'{script_directory}','MetaCC', 'Auxiliary', 'FragGeneScan', 'FragGeneScan')
    os.system("chmod 777 " + fragURL)
    fragScanURL = os.path.join(f'{script_directory}','MetaCC','Auxiliary', 'FragGeneScan', 'run_FragGeneScan.pl')
    os.system("chmod 777 " + fragScanURL)
    hmmExeURL = os.path.join(f'{script_directory}','MetaCC','Auxiliary', 'hmmer-3.3.2', 'bin', 'hmmsearch')
    os.system("chmod 777 " + hmmExeURL)
    markerExeURL = os.path.join(f'{script_directory}','MetaCC','Auxiliary' , 'test_getmarker.pl')
    os.system("chmod 777 " + markerExeURL)
    markerURL = os.path.join(f'{script_directory}','MetaCC','Auxiliary' , 'marker.hmm')
