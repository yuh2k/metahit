...
def fast_norm_tipbased_bylength(coords, data, tip_lengths, tip_size):
    ...
    for ii in range(coords.shape[1]):
        i = int(coords[0, ii])  # Cast first coordinate as int
        j = int(coords[1, ii])  # Cast second coordinate as int
...
def fast_norm_tipbased_bysite(coords, data, sites):
    ...
    for n in range(coords.shape[1]):
        # Cast the coordinates to integers explicitly by accessing individual elements
        i = int(coords[0, n].item())  # First dimension of the COO matrix coordinates
        j = int(coords[1, n].item())  # Second dimension of the COO matrix coordinates
        k = int(coords[2, n].item())  # Third dimension of the COO matrix coordinates
        l = int(coords[3, n].item())  # Fourth dimension of the COO matrix coordinates
...
def fast_norm_fullseq_bysite(rows, cols, data, sites):
    ...
    for n in range(data.shape[0]):
        i = int(rows[n])  # Cast to int
        j = int(cols[n])  # Cast to int
...
class ExtentGrouping:
    ...
    def __init__(self, seq_info, bin_size):
        ...
        for n, seq in tqdm.tqdm(enumerate(seq_info), total=len(seq_info), desc='Making bins'):
            ...
            edges = np.linspace(0, seq.length, num_bins+1, endpoint=True, dtype=int)
            ...
            self.borders.append(np.array([first_bin, last_bin], dtype=int))
...
class SeqOrder:
    ...
    STRUCT_TYPE = np.dtype([('pos', int), ('ori', int), ('mask', np.bool_), ('length', int)])
    ...
    def asindex(_ord):
        ...
        return np.array(zip(_ord, np.ones_like(_ord, dtype=np.bool_)), dtype=SeqOrder.INDEX_TYPE)
    ...
    def set_mask_only(self, _mask):
        ...
        """
        _mask = np.asarray(_mask, dtype=np.bool_)
    ...
    def set_order_and_orientation(self, _ord, implicit_excl=False):
        ...
        if len(_ord) < len(self.order):
            ...
            _mask = np.zeros_like(self.mask_vector(), dtype=np.bool_)
...
class ContactMap:
    ...
    def __init__(self, bam_file, enzymes, seq_file, min_insert, min_mapq=0, min_len=0, min_sig=1, min_extent=0,
        ...
        with pysam.AlignmentFile(bam_file, 'rb') as bam:
            ...
            self.current_mask = np.ones(self.total_seq, dtype=np.bool_)
    ...
    def set_primary_acceptance_mask(self, min_len=None, min_sig=None, max_fold=None, update=False):
        ...
        acceptance_mask = np.ones(self.total_seq, dtype=np.bool_)
    ...
    def prepare_seq_map(self, norm=True, bisto=False, mean_type='geometric'):
        ...
        _map = self.seq_map.astype(float)
    ...
    def get_extent_map(self, norm=True, bisto=False, permute=False, mean_type='geometric'):
        ...
        _map = self.extent_map.astype(float)
    ...
    def _get_sites(self):
        _sites = np.array([si.sites for si in self.seq_info], dtype=float)
    ...
    def _norm_extent(self, _map, mean_type='geometric'):
        ...
        if _map.dtype not in {float, float}:
            _map = _map.astype(float)
        ...
        for row_i, col_dat in enumerate(_map.rows):
            ...
            wi = np.fromiter((1e-3 * _mean_func(_len[i], _len[j])
                              for j in np.searchsorted(_cbins, col_dat, side='right')), dtype=float)
            _map.data[row_i] /= wi
        return _map
        ...
        if pattern_only:
            ...
            if max_image_size is not None:
                ...
                if np.max(full_size) > max_image_size:
                    ...
                    tick_locs = np.floor(tick_locs.astype(float) / reduce_factor)
