...
mdmcleaner=false
# load in params
OPTS=`getopt -o ht:m:o:x:c:l:b:1:2: --long help,parallel,skip-checkm2,strict-cut-off,permissive-cut-off,nanopore,mdmcleaner -- "$@"`
...
if [ ! -s $scriptdir/sort_contigs.py ]; then
	error "The folder $scriptdir doesnt exist. Please make sure config.sh is in the same filder as the mains scripts and all the paths in the config.sh file are correct"
fi

echo "comp : $comp; cont : $cont"
########################################################################################################
########################               BEGIN REASSEMBLY PIPELINE!               ########################
########################################################################################################

conda activate /home/ndx163/miniconda3/envs/test_metahit4
...
if [[ ! -s ${out}/binned_assembly/assembly.fa.amb ]]; then
	comm "Indexing the assembly"
        cat ${out}/binned_assembly/assembly.fa | tr -d " " > ${out}/binned_assembly/assembly.tmp
        rm -f ${out}/binned_assembly/assembly.fa
        mv ${out}/binned_assembly/assembly.tmp ${out}/binned_assembly/assembly.fa
	...
	comm "Aligning all reads back to entire assembly and splitting reads into individual fastq files based on their bin membership"
		...
		| python ${scriptdir}/filter_nanopore_reads_for_bin_reassembly.py ${out}/original_bins ${out}/reads_for_reassembly
	fi

	bwa mem -t $threads ${out}/binned_assembly/assembly.fa $f_reads $r_reads \
	 | python ${scriptdir}/filter_reads_for_bin_reassembly.py ${out}/original_bins ${out}/reads_for_reassembly $strict_max $permissive_max
...
for i in $( ls ${out}/reassemblies/ ); do
	...
	if [ -s ${out}/reassemblies/${bin_name}/scaffolds.fasta ]; then
		python ${scriptdir}/rm_short_contigs.py $len\
...
conda activate /home/ndx163/miniconda3/envs/test_metahit4_checkm2
...
if [ "$run_checkm2" = true ]; then
	...
	mkdir ${out}/tmp
	$checkm2script predict -x fa -i ${out}/reassembled_bins -o ${out}/reassembled_bins.checkm2 -t $threads --tmpdir ${out}/tmp 
	if [[ ! -s ${out}/reassembled_bins.checkm2/quality_report.tsv  ]]; then error "Something went wrong with running checkm2. Exiting..."; fi
	python ${scriptdir}/summarize_checkm2.py ${out}/reassembled_bins.checkm2/quality_report.tsv | (read -r; printf "%s\n" "$REPLY"; sort) > ${out}/reassembled_bins.stats
	...
	for i in $(python ${scriptdir}/choose_best_bin.py ${out}/reassembled_bins.stats $comp $cont); do 
	...
	if [[ $(ls ${out}/reassembled_best_bins | wc -l) -gt 0 ]]; then 
		...
		mv ${out}/reassembled_bins.checkm2 ${out}/work_files/
	...
	rm -r ${out}/tmp
        ...
        checkm2 bin_qa_plot -x fa ${out}/reassembled_bins.checkm2 ${out}/reassembled_bins ${out}/reassembled_bins.plot
        if [[ ! -s ${out}/reassembled_bins.plot/bin_qa_plot.png ]]; then warning "Something went wrong with making the checkm2 plot. Exiting."; fi
	...
	${scriptdir}/plot_reassembly.py $out $comp $cont ${out}/reassembled_bins.stats ${out}/original_bins.stats
