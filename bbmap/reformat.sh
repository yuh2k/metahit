...
tag=                    Look for this tag in the header to filter by the next value.
                        e.g. to filter reads with a tag like 'depth=5.5' where you only want depths
                        of at least 3, the two necessary flags would be 'tag=depth= minvalue=3'.
delimiter=              Character after the end of the value, such as delimiter=X.  Control symbols
                        may be escaped or spelled out, like delimiter=\t or delimiter=pipe.
                        The tag may contain the delimiter.  If the value is the last term
                        in the header, the delimiter doesn't matter but is still required.
